...
```hcl
module "virtual-gateway" {
    source = "github.com/pbs/terraform-virtual-gateway-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/virtual-gateway'
gh -R 'pbs/terraform-virtual-gateway-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/virtual-gateway'
rm -f release.tar.gz
```
...
```bash
git remote add -f virtual-gateway git@github.com:pbs/terraform-virtual-gateway-module.git
git subtree add --prefix terraform/modules/virtual-gateway virtual-gateway main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/virtual-gateway virtual-gateway $REF --squash
```
